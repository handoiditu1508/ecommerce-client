...
import { addToCart, cartSelectors, changeProductVariantInCart, removeFromCart, setQuantityForCart } from "../slices/cartSlice";
...
const registerCartListener = (startAppListening: TypedStartListening<RootState, AppDispatch, unknown>) => {
  startAppListening({
    ...
    effect: async (action, api) => {
      ...
      const isCartHydrated = cartSelectors.hydrated(state);
